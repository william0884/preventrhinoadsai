  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const result = await response.json() as GalleryItem[];
              ...
              <>
                <Link href={`/gallery/${item.id}`} passHref>
                  <Image
                    ...
                    width={480}
                    height={480} // Add an appropriate height
                    priority
                  />
                </Link>
                <div className="flex space-x-4 mt-2">
                  <button
                    onClick={() => item.url && handleDownload(item.url, item.id)}
                    className="flex items-center px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors"
                  >
                    <FaDownload className="mr-2" /> Download
                  </button>
                  <button
                    onClick={() => item.url && handleShare(item.url, item.sentence)}
